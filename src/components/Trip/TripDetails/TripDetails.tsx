    ...
    return (
        <Modal open={open} onCancel={onClose} footer={null} title="Подробности поездки">
            <div className='modal-body'>
                ...
                <h3>Даты</h3>
                <Descriptions size="small" bordered className='table-overflow' layout="vertical">
                    <Descriptions.Item label="Дата">{trip.date ? trip.date : "Нет данных"}</Descriptions.Item>
                        ...
                        <Descriptions size="small" bordered className='table-overflow' layout="vertical">
                            <Descriptions.Item label="Местонахождения">
                                {trip.location_address ? (
                                    <>
                                        {trip.location_address}
                                        <a href={`https://yandex.ru/maps/?text=${trip.location_address}`} target='_blank'>Перейти в Яндекс.Карты</a>
                                    </>
                                ) : "Нет данных"}
                            </Descriptions.Item>
                            ...
                            <Descriptions.Item label="Назначения">
                                {trip.destination_address ? (
                                    <>
                                        {trip.destination_address}
                                        <a href={`https://yandex.ru/maps/?text=${trip.destination_address}`} target='_blank'>Перейти в Яндекс.Карты</a>
                                    </>
                                ) : "Нет данных"}
                            </Descriptions.Item>
                        </Descriptions>
                ...
                {trip.price && (
                    <>
                        <h3>Стоимость</h3>
                        <Descriptions size="small" bordered className='table-overflow' layout="vertical">
                            <Descriptions.Item label="Цена">{trip.price.price ? trip.price.price : "Нет данных"}</Descriptions.Item>
                        </Descriptions>
                    </>
                )}
            </div>
